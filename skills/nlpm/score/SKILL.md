---
name: score
description: "Score NL programming artifacts — 100-point quality score per file. Use when the user invokes $nlpm-score or requests this exact NLPM workflow."
---

# $nlpm-score

Treat the text following `$nlpm-score` in the user's request as the command arguments. Preserve empty input as empty; never invent flags.
...
Read `.codex/nlpm.local.md` if it exists. Extract `score_threshold` (default: 70).
...
If `--changed` is present: run `git diff --name-only HEAD` to get changed files, then filter through `skills/nlpm/workflow-core/references/classify.md` to keep only NL artifacts. Skip the full discovery step.

### Step 3: Discover Artifacts

If path is a directory: use `skills/nlpm/workflow-core/references/discover.md` to discover all NL artifacts.
If path is a file: use `skills/nlpm/workflow-core/references/classify.md` to determine its type.
...
1. **`scorer`** -- scores each artifact on the 100-point rubric (penalties, structure, heuristics)
2. **`vague-scanner`** -- counts vague quantifier words mechanically
...
- Any rule overrides from the config (if `rule_overrides` is present in `.codex/nlpm.local.md`)
...
Persist this scoring run to `.codex/nlpm-history.json` so `$nlpm-trend` has data to compare against. Follow `skills/nlpm/workflow-core/references/append-history.md` with:
