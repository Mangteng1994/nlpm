---
name: spec-sync
description: "Sync nlpm's convention overlays with the latest official Claude Code / Codex / Antigravity specs — research upstream docs, diff, apply corrections, propagate, check. Use when the user invokes $nlpm-spec-sync or requests this exact NLPM workflow."
---

# $nlpm-spec-sync

Treat the text following `$nlpm-spec-sync` in the user's request as the command arguments. Preserve empty input as empty; never invent flags.
...
A correction to an overlay can invalidate nlpm's *own* artifacts (e.g. an agent using a now-removed color or tool name). For each `FIX`/`REMOVE` that changed an enum value, field name, or tool name, grep the rest of the repo for the old value with a **fixed-string** search — old values like `[profiles.*]`, legacy dispatcher syntax, or `color: magenta` may contain regex metacharacters and would misbehave under `grep -E`:
