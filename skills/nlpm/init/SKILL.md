---
name: init
description: "Prepare NLPM for this project — detect artifacts, set strictness, capture baseline trend snapshot. Use when the user invokes $nlpm-init or requests this exact NLPM workflow."
---

# $nlpm-init

Treat the text following `$nlpm-init` in the user's request as the command arguments. Preserve empty input as empty; never invent flags.

## Workflow

### Step 1: Check Existing Config

Read `.codex/nlpm.local.md`. If exists, show current settings and ask: "Reconfigure? (yes/no)". If no → stop.
...
Create `.codex/` if needed. Write `.codex/nlpm.local.md`. If only `.claude/nlpm.local.md` exists, read it once as migration input, preserve recognized settings, and leave it untouched:
...
1. Delegate to `scorer` and `vague-scanner` in parallel over the discovered artifacts (same dispatch pattern as `$nlpm-score`). The vague-scanner's counts override the scorer's heuristic detection.
2. Persist the result by following `skills/nlpm/workflow-core/references/append-history.md` with `scope: "full"` and `files_scored` equal to the artifact count.
...
NLPM is ready for this project.
  ...
  Threshold: {70}/100
  Artifacts found: {N}
  Baseline score: {overall}/100   (snapshot saved to .codex/nlpm-history.json)
  Config: .codex/nlpm.local.md
...
- `.codex/` directory doesn't exist → create it
