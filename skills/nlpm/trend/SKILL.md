---
name: trend
description: "Show quality score trends over time — track improvements, detect degradation. Use when the user invokes $nlpm-trend or requests this exact NLPM workflow."
---

# $nlpm-trend

Treat the text following `$nlpm-trend` in the user's request as the command arguments. Preserve empty input as empty; never invent flags.
...
Read `.codex/nlpm-history.json` from the project root.
...
Persist this run by following `skills/nlpm/workflow-core/references/append-history.md` with the scope determined in Step 3, the per-file scores from Step 2, and the file count. The partial handles file creation, deduplication, and atomic write.
...
--------------------------------------------------------------
.codex/agents/scorer.toml                  95      92        +3 improved
.codex/agents/scanner.toml                 90      90         0 unchanged
skills/nlpm/score/SKILL.md                 95      88        +7 improved
skills/nlpm/scoring/SKILL.md      85      85         0 unchanged
.codex/agents/tester.toml                  78      82        -4 degraded
skills/nlpm/fix/SKILL.md (NEW)             88      --        new

Overall: 88/100 (was 87, +1)

Degraded (needs attention):
  .codex/agents/tester.toml  82 → 78 (-4)
