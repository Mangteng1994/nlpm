---
name: vocab-init
description: "Create a vocabulary skill for a Codex plugin or NL-artifact-heavy repository: detect layout, extract literary warrant, seed canonical noun/verb tables, and write the R51 opt-in. Use when the user invokes $nlpm-vocab-init or requests this exact workflow."
---

# $nlpm-vocab-init

Treat the text following `$nlpm-vocab-init` in the user's request as the command arguments. Preserve empty input as empty; never invent flags.
...
Read `<target>/.codex-plugin/plugin.json` if it exists and use its `name`. A legacy `.claude-plugin/plugin.json` may be read only when no Codex manifest exists because it is audit input, not a runtime dependency.
...
|---------|-------|------|-------|
| `skills/*/SKILL.md` | internal | skill-dirname | Codex plugin skills |
| `.codex/agents/*.toml` | internal | noun-filename | Codex subagents |
| `commands/*.md` | internal | verb-filename | Static Claude command audit corpus |
| `agents/*.md` | internal | noun-filename | Static Claude agent audit corpus |
...
1. `${PLUGIN_ROOT}/analysis/scripts/extract-vocabulary.py` — set when the command runs from within NLPM
2. Fall back to resolving `analysis/scripts/extract-vocabulary.py` relative to this skill's repository root. Do not probe another tool's plugin cache.
...
---
name: vocabulary
description: "Use when writing, reviewing, or naming any {plugin_name} artifact — pick the canonical noun or verb from this registry rather than coining a synonym. Loaded by NLPM's scorer and checker when R51 is enabled in .codex/nlpm.local.md."
version: 0.1.0
---
...
R51 is **off by default**. To enable vocabulary drift detection on this project, add to `.codex/nlpm.local.md`:
...
Next steps:
  ...
  4. Opt in by adding to .codex/nlpm.local.md:
...
| NLPM root not findable | "Could not locate NLPM's extract-vocabulary.py. Install the Codex NLPM plugin or run from its repository." |
