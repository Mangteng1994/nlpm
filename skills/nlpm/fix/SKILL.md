---
name: fix
description: "Auto-fix fixable findings in NL artifacts — missing fields, heading gaps, field renames. Use when the user invokes $nlpm-fix or requests this exact NLPM workflow."
---

# $nlpm-fix

Treat the text following `$nlpm-fix` in the user's request as the command arguments. Preserve empty input as empty; never invent flags.
...
Parse the invocation arguments for a path (default: cwd if empty). Delegate to the `scorer` subagent to score all artifacts at that path. Collect all findings.
